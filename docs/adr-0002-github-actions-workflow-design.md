...
- 同workflowから`wf-lint-gha.yaml`を呼び出し、`ne-sachirou/lint-gha-reviewdog`でGitHub Actionsを檢証する。fork PRでは書込みを必要としない`local` reporterを使い、同一PRの新しい實行は古い實行をcancelする。
...
- 連續pushでは最新の實行だけを維持し、cacheがない初回實行にもtimeoutの余裕を持たせる。
